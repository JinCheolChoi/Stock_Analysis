      ...
      Time_Elapsed=system.time({
        Results=lapply(Trading_Dates,
                       ...
                       function(x){
                         ...
                         #                   Time<paste0(x, " ", Market_Close_Time), ]
                         BarData=BarData[Time>=paste0(x, " ", as.ITime(Market_Open_Time)+60*60)& # remove the first 1 hour after market open
                                           Time<paste0(x, " ", Market_Close_Time), ]
                         BarData[, Ind:=.I]
...
twsCALLBACK_cust=function (twsCon, eWrapper, timestamp, file, playback = 1, ...) 
{
  ...
  if (inherits(twsCon, "twsPlayback")) {
    ...
    tryCatch(while (isConnected(twsCon) & rep.ind==1) { # # # #
      ...
      if(is.null(curMsg) # # # # Sometimes, curMsg is not derived
      ){
        curMsg=50
      }else if(curMsg!=50){
        Sys.sleep(0.5) # suspend execution for a while to prevent the system from breaking
...
eWrapper_cust=function (debug = FALSE, errfile = stderr()) 
{
  ...
  if (is.null(debug)) {
    ...
    marketDataType <- positionData <- positionEnd <- accountSummary <- accountSummaryEnd <- verifyMessageAPI <- verifyCompleted <- displayGroupList <- displayGroupUpdated <- symbolSamples <- function(curMsg, 
                                                                                                                                                                                                        msg, timestamp, file, ...) {
      c(curMsg, msg)
    }
  }
  ...
  eW <- list(.Data = .Data, get.Data = get.Data, assign.Data = assign.Data, 
             ...
             realtimeBars = realtimeBars, currentTime = currentTime, 
             fundamentalData = fundamentalData, deltaNeutralValidation = deltaNeutralValidation, 
             tickSnapshotEnd = tickSnapshotEnd, marketDataType = marketDataType, 
             commissionReport = commissionReport, positionData = positionData, 
             positionEnd = positionEnd, accountSummary = accountSummary, 
             accountSummaryEnd = accountSummaryEnd, verifyMessageAPI = verifyMessageAPI, 
             verifyCompleted = verifyCompleted, displayGroupList = displayGroupList, 
             displayGroupUpdated = displayGroupUpdated, symbolSamples = symbolSamples)
  class(eW) <- "eWrapper"
  invisible(eW)
}
  ...
  if(BarSize>=30){
    ...
    if(Counting_Down==TRUE){
      ...
      if((BarSize-round(as.numeric(Sys.time())%%BarSize))-Seconds_Before_Proceeding>0){
        print(paste0("wait for the next bar starting in"))
        while((BarSize-round(as.numeric(Sys.time())%%BarSize))-Seconds_Before_Proceeding>0){
          print(paste0((BarSize-round(as.numeric(Sys.time())%%BarSize)), " second(s)"))
